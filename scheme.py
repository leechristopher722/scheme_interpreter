...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operands_lst = expr.rest.map(lambda x: scheme_eval(x, env))
...
def do_and_form(expressions, env):
    ...
    if expressions.rest is nil and is_true_primitive(current_eval):
        return current_eval
    if is_false_primitive(current_eval):
        return False
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        return False
    current_eval = scheme_eval(expressions.first, env)
    if is_true_primitive(current_eval):
        return current_eval
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        current_set = bindings.first
        validate_form(current_set, 2, 2)
        names = Pair(current_set.first, names)
        values = Pair(scheme_eval(current_set.rest.first, env), values)
